function [tmpParticle,Weight] = PF_Inference_Body_Pekee(Particle,OriNumOfProposal,OriAffMatrix,OriPosMatrix);
% For robot, we assume the affinity is symmetric and it only contains two
...
for PF_Idx = 1:length(Particle)
   ...
   % AffMatrix(:,TakenLabel,:) = 0;% Taken Label should not be considered any more
