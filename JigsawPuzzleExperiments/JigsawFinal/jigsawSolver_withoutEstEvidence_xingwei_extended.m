...
noFixStep = 2;%[0:2:10];   % This number specifies how many patches to fix to correct locations when we reconstruct the image
nIter = 100;            % The number of BP iterations
randInit = 1;           % Random BP message initialization
pairCorr = zeros(10,noFixStep);   % For measuring the correct image reconstruction rate according to pair-wise correct measure
clusterCorr = zeros(10, noFixStep);% For measuring the correct image reconstruction rate according to cluster-wise correct measure
neighCorr = zeros(10, noFixStep);  % For measuring the correct image reconstruction rate according to neighbor-wise correct measure
ReConstructImage = cell(10, noFixStep);

%% load image
imNo = 1;
ImExtended = dir('C:\yxw\CVPR 2011\jigsawCode\jigsawCode\imData_Extended2\*.jpg');
for imNo = 1:length(ImExtended)
    imName =['C:\yxw\CVPR 2011\jigsawCode\jigsawCode\imData_Extended2\', ImExtended(imNo).name];
    ...
    for noFixInd = 1:2%:length(noFixStep)
        ...
        for BPIter = 1:noBPIter
           ...
           % figure, imshow(imOutTemp)
            ...
            save Jigsaw_MIT_Patch48_OneFix_extended2 pairCorr clusterCorr neighCorr ReConstructImage;
        end
    end
end
% % imNo
