...
minDistInd = randperm(N_patches*M_patches);
...
for noFixInd = 6:length(noFixStep)
    ...
    %% Get the initial labels
     RIdx = randperm(noPatches);
    Labels = corrPatchTemp'; Labels = Labels(:);
    %swaping the labels for some nodes
    
    T = Labels(RIdx(1:10));
    Labels(RIdx(1:10)) = Labels(RIdx(301:310));
    Labels(RIdx(301:310)) = T;
    ...
    for BPIter = 1:noBPIter
        ...
        [nodes] = initBPMessagesWConst_Xingwei(nodes, K_states, exclusive, randInit,Labels);
