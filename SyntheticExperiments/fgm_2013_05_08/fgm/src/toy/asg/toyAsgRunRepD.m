...
for iBin = 1 : nBin
    ...
    %.
    %September 10, 2013.
    return_params=SMC_CMWS_Matching_Affinity(K,size(Ct,1),size(Ct,2),800,10);
    smcsp.obj=return_params.mws;
    smcsp.acc=matchAsg(return_params.P, asgT);
    %Acc(end+1,iBin)=smcsp.acc;
    %Obj(end+1,iBin)=smcsp.obj;
    %.
    %September 11, 2013.
    Acc(nAlg+1,iBin)=smcsp.acc;
    Obj(nAlg+1,iBin)=smcsp.obj;
end
