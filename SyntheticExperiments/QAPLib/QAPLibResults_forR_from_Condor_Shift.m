...
fid=fopen(sprintf('%s/SMCSPQAPResultsFromCondorShifted.csv',qaplib_root),'w');
...
for i=1:length(result_files)
    ...
    if(strcmpi('chr',progInstance(1:3))==1)
        if(return_params.QAPObj<chr_shift)
            QAPObj=QAPObj+chr_shift-return_params.QAPObj;
            return_params.QAPObj=chr_shift;
        end
    end
    if(strcmpi('esc',progInstance(1:3))==1)
        if(return_params.QAPObj<esc_shift)
            QAPObj=QAPObj+esc_shift-return_params.QAPObj;
            return_params.QAPObj=esc_shift;
        end
    end
    if(strcmpi('tai',progInstance(1:3))==1)
        if(return_params.QAPObj<tai_shift)
            QAPObj=QAPObj+tai_shift-return_params.QAPObj;
            return_params.QAPObj=tai_shift;
        end
    end
    if(strcmpi('nug',progInstance(1:3))==1)
        if(return_params.QAPObj<nug_shift)
            QAPObj=QAPObj+nug_shift-return_params.QAPObj;
            return_params.QAPObj=nug_shift;
        end
    end
