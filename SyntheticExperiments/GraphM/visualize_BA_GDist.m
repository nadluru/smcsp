...
idx=1:18;
%SMC vs Umeyama
savefig_wrapper_BA(SMC_MWS_GD,U_GD,sprintf('n=%d, # of particles=%d',length(idx),num_par),'GDist (Avg. (SMC,Umeyama)) [arb. unit]','GDist (SMC-Umeyama) [arb. unit]',save_flag,sprintf('SMC_%d_vs_Umeyama_GD.eps',num_par));
savefig_wrapper_BA(SMC_MWS_GD,RANK_GD,sprintf('n=%d, # of particles=%d',length(idx),num_par),'GDist (Avg. (SMC,RANK)) [arb. unit]','GDist (SMC-RANK) [arb. unit]',save_flag,sprintf('SMC_%d_vs_RANK_GD.eps',num_par));
savefig_wrapper_BA(SMC_MWS_GD,QCV_GD,sprintf('n=%d, # of particles=%d',length(idx),num_par),'GDist (Avg. (SMC,QCV)) [arb. unit]','GDist (SMC-QCV) [arb. unit]',save_flag,sprintf('SMC_%d_vs_QCV_GD.eps',num_par));
savefig_wrapper_BA(SMC_MWS_GD,PATH_GD,sprintf('n=%d, # of particles=%d',length(idx),num_par),'GDist (Avg. (SMC,PATH)) [arb. unit]','GDist (SMC-PATH) [arb. unit]',save_flag,sprintf('SMC_%d_vs_PATH_GD.eps',num_par));

load('Other_Algo_GDists_18.mat');
save_flag=false;
idx=1:18;
num_par=[200 400 600 800];
mu=zeros(length(num_par),4);
xmin=zeros(length(num_par),4);
xmax=zeros(length(num_par),4);
for i=1:length(num_par)
    load(sprintf('SMC_MWS_GD_with_dummy_%d.mat',num_par(i)));
    %SMC vs Umeyama
...
colors{1}='r';colors{2}='g';colors{3}='b';colors{4}='c';
