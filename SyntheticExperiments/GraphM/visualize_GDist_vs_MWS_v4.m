...
for num_samples=[10 100 1000]% 10000]
    ...
    xlabel('Weight of sub-graph of A [arb. unit]');%ylabel('exp(-GDist between A_1 and A_2) [arb. unit]');
    ylabel('GDist between A_1 and A_2 [arb. unit]');
    savefig(sprintf('MWS_vs_GDist_4_%d.eps',num_samples),2);
    %savefig(sprintf('MWS_vs_GDist_4_%d.png',num_samples),0);
end